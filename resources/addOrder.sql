...
DELIMITER $$
USE `storefront`$$
CREATE DEFINER=`dev`@`localhost` PROCEDURE `addOrder`(IN orderDate DATETIME, IN orderDetails JSON, OUT orderId INT, OUT insertedRecords INT)
BEGIN
	DECLARE i INT DEFAULT 0;
    DECLARE num_items INT;
    DECLARE item_description VARCHAR(255);
    DECLARE qty INT;
    SET num_items = JSON_LENGTH(ordaddOrdererDetails);
