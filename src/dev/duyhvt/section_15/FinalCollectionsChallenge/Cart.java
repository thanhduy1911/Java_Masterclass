package dev.duyhvt.section_15.FinalCollectionsChallenge;
...
public class Cart {
    ...
    public void addItem(InventoryItem item, int qty) {
        ...
        if (!item.reserveItem(qty)) {
            System.out.println("Ouch, something went wrong, could not add " + item.getProduct().sku());
        }
    }
    ...
    public void removeItem(InventoryItem item, int qty) {
        ...
        if (current < qty) {
            ...
            System.out.printf("%d [%s] removed%n", qty, item.getProduct().name());
        }
