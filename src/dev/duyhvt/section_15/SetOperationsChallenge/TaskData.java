...
//            Priority priority = Priority.valueOf(data[2].toUpperCase());
            ...
            Status status;
            try {
                status = (data.length <= 3) ? Status.IN_QUEUE :
                        Status.valueOf(data[3].toUpperCase()
                                .replace(' ', '_'));
            } catch (IllegalArgumentException e) {
                status = Status.IN_QUEUE; // if anything went wrong, set to default value logic
            }

            Priority priority;
            try {
                priority = Priority.valueOf(data[2].toUpperCase());
            } catch (IllegalArgumentException e) {
                priority = Priority.LOW; // if anything went wrong, set to default value logic
            }
